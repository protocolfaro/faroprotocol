...
def _buscar_s2_stac(area: dict, dias: int = 60, max_nubes: float = 50.0) -> dict | None:
    """
    Fallback STAC cuando CDSE falla o no tiene cuota.
    Intenta CDSE-STAC → Planetary Computer → Element84 (en ese orden).
    Retorna el primer ítem STAC encontrado o None.
    """
    try:
        sys.path.insert(0, str(PROJECT_ROOT))
        from faro_data_fetcher import search_stac
        ...
        results = search_stac(
            ...
            max_cloud=int(max_nubes),
            days_back=dias,
            limit=3,
        )
        return results[0] if results else None
    except Exception as e:
        print(f'  [STAC-fallback] Error: {e}')
        return None


def _descargar_banda_stac(url: str, dest: Path, needs_sign: bool = False) -> Path:
    """Descarga una banda COG/JP2 desde URL STAC directamente."""
    if needs_sign:
        try:
    ...
    r.raise_for_status()
    mb_total = int(r.headers.get('content-length', 0)) / 1e6
    print(f'    Descargando {dest.name} ({mb_total:.0f} MB)...')
...
def _pipeline_s2_stac(area: dict, stac_item: dict, ndvi_out: Path) -> Path | None:
    """Orquesta descarga directa de bandas desde STAC y cálculo NDVI."""
    assets = stac_item.get('assets', {})
    # Mapeo de nombres de asset según catálogo (CDSE usa B04/B08, PC/E84 igual)
    b04_url = assets.get('B04') or assets.get('red') or assets.get('B4')
    b08_url = assets.get('B08') or assets.get('nir') or assets.get('B8')

    if not b04_url or not b08_url:
        print(f'  [STAC] Assets B04/B08 no disponibles en {stac_item["source"]} — sin fallback')
        return None

    needs_sign = stac_item.get('needs_sign', False)
    prefix     = stac_item['id'][:30].replace('/', '_')
    stac_dir   = DATOS_S2 / 'stac'

    print(f'  [STAC] Fuente: {stac_item["source"]} — {stac_item["id"][:50]}')
    print(f'  [STAC] Fecha: {stac_item.get("datetime","?")[:10]} | '
          f'Nubosidad: {stac_item.get("cloud_cover","?"):.0f}%'
          if isinstance(stac_item.get('cloud_cover'), (int, float))
          else f'  [STAC] Fecha: {stac_item.get("datetime","?")[:10]}')

    b04_dest = stac_dir / f'{prefix}_B04.tif'
    b08_dest = stac_dir / f'{prefix}_B08.tif'

    # Reusar si ya descargado
    if not b04_dest.exists():
        _descargar_banda_stac(b04_url, b04_dest, needs_sign)
    else:
        print(f'    B04 ya existe: {b04_dest.name}')

    if not b08_dest.exists():
        _descargar_banda_stac(b08_url, b08_dest, needs_sign)
    else:
        print(f'    B08 ya existe: {b08_dest.name}')

    print(f'\n  [S2-STAC-4] Calculando NDVI (CLOSDI shadow filter)...')
    try:
        ndvi_path = calcular_ndvi_y_guardar(b04_dest, b08_dest, area['bounds'], ndvi_out)
        return ndvi_path
    except Exception as e:
        print(f'  ERROR en NDVI (STAC): {e}')
        return None
...
def pipeline_s2(area_name: str,
    ...
    # 1. Buscar — CDSE primero, STAC como fallback soberano
    print(f'\n  [S2-1] Buscando escena L2A (≤{max_nubes}% nubes, últimos {dias} días)...')
    producto = None
    try:
        producto = buscar_s2(area['polygon'], dias=dias, max_nubes=max_nubes)
    except Exception as e:
        print(f'  [CDSE] Error en búsqueda: {e}')

    if not producto:
        print(f'  [CDSE] Sin resultados — activando Motor de Respaldo STAC...')
        stac_item = _buscar_s2_stac(area, dias=dias, max_nubes=max_nubes)
        if stac_item:
            return _pipeline_s2_stac(area, stac_item, ndvi_out)
        print(f'  SIN DATOS: CDSE + todos los catálogos STAC sin resultados')
        return None
