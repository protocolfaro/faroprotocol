...
_TAREAS_BASE = [
    ...
    ["Aerificar porterías Amalfitani", "Inspección general campo principal"],
    ["Inspección Poli Feijóo — básquet y playón norte", "Riego mantenimiento 1FA y 3FA"],
    ["Revisión integral — VO + Amalfitani + Poli", "Control sistema de riego todos los campos"],
]
...
def write_velez_data(output_path: str = None, fecha: str = None, live: bool = True) -> str:
    ...
    # Preserve keys generated by other pipelines (shadow_analysis, IPOS heatmaps)
    _PRESERVE_TOP  = ("shadow_maps",)
    _PRESERVE_USER = ("heatmaps", "heatmaps_meta", "heatmap_gps_overlay")
    if os.path.exists(output_path):
        try:
            ...
            for uid, udata in existing.get("usuarios", {}).items():
                if uid in data.get("usuarios", {}):
                    for ukey in _PRESERVE_USER:
                        if ukey in udata and ukey not in data["usuarios"][uid]:
                            data["usuarios"][uid][ukey] = udata[ukey]
        except Exception:
            pass
