...
if _vd_path:
    try:
        with open(_vd_path, encoding="utf-8") as _f:
            _vd = _json.load(_f)
        _vd_s = _vd.get("sectores", {})
        _canchas = _vd_s.get("canchero", {}).get("canchas", [])
        if _canchas:
            ...
            ZONES = [{
                ...
                'accion': (c.get("detalle", "").split('\xb7')[1].strip()
                           if '\xb7' in c.get("detalle", "")
                           else c.get("detalle", "").split('·')[1].strip()
                           if '·' in c.get("detalle", "")
                           else c.get("detalle", "")),
            } for i, c in enumerate(_canchas[:5])]
        _solar = _vd_s.get("solar", {})
...
out = str(_out_path or (_REPORT_DIR / 'faro_reporte_velez.png'))
