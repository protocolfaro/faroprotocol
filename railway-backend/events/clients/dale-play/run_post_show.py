...
def run_post_show(show_id: str) -> dict:
    ...
    lines = [
        "",
        "=" * 60,
        "DALE PLAY - POST-SHOW PIPELINE - RESULTADOS",
        "=" * 60,
        f"Show:          {show_id}",
    ...
    lines += [
        ...
        "",
        "-- OUTPUTS " + "-" * 49,
        f"  PNG estandar:     {result.get('report_png', 'N/D')}",
        f"  PNG comparativa:  {png_comp}",
        f"  PDF certificado:  {cert.get('pdf_path', 'N/D')}",
        f"  SHA-256:          {cert.get('cert_hash', 'N/D')}",
        "=" * 60,
    ]
